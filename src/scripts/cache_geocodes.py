...
def cache_all_user_ents():
    print('Initializing .....')
    cache_filepath = os.path.join(ROOT_DIR, 'cache', 'user_ents_cache.pk')
    nlp = get_nlp()
    connect_to_mongo()
...
def cache_geocodes_from_ents():
    '''Memoize a map from entities to geocodes in a file.'''
    user_ents_fp = os.path.join(ROOT_DIR, 'cache', 'user_ents_cache.pk')
    user_ents = pickle.load(open(user_ents_fp, 'rb'))
    session = requests.Session()
    cache_filepath = os.path.join(ROOT_DIR, 'cache', 'geonames_geocodes_cache.pk')
    ...
    for e in tqdm.tqdm(ents):
        if e not in ent_geocode_map:
            ent_geocode_map[e] = get_geocodes(e, session=session)
            pickle.dump(ent_geocode_map, open(cache_filepath, 'wb'))


def cache_geocodes_from_maps():
    '''Cache all hardcoded alias names for places.'''
    session = requests.Session()
    cache_filepath = os.path.join(ROOT_DIR, 'cache', 'geonames_geocodes_cache.pk')
    gazetteer = pd.read_csv(os.path.join(ROOT_DIR, 'resources', 'gazetteer.csv'))
    state_abbrev_map = map_state_abbrevs(gazetteer)
    ...
    ents = list(state_abbrev_map.values()) + list(ALIAS_MAP.values()) + list(it.chain(*LARGE_STATE_MAP.values()))
...
def main():
    # utility variables
    session = requests.Session()
    cache_filename = os.path.join(ROOT_DIR, 'cache', 'geocodes_cache.pk')
    ent_cache_filename = os.path.join(ROOT_DIR, 'cache', 'ents_cache.csv')
    nlp = get_nlp()
    connect_to_mongo()
