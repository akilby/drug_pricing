...
parser.add_argument("--complete_threads_file", default='use_data/threads/all_dumps.csv')
parser.add_argument("--complete_comments_file", default='use_data/comments/all_comments.csv')
...
def main():
    ...
    meth_words, sub_words, narc_words, nalt_words = generates_non_case_sensitive_list_of_keyterms(mat_filepath)
    ...
    print('-' * 100)
    print('PUT POSTS INTO DATA FRAME AND FILTER FOR KEYWORDS')
    print('-' * 100)

    state_init_df = convert_key_word_posts_to_df(total_posts, locations, general_re, name_of_search_for = 'state_init', sep='|', case_sensitive=False, search_for_chunksize=25)
    price_df = convert_key_word_posts_to_df(total_posts, units, price_re, name_of_search_for = 'price', sep='|', case_sensitive=False, search_for_chunksize=25)
    unit_df = convert_key_word_posts_to_df(total_posts, units, general_re, name_of_search_for = 'units', sep='|', case_sensitive=False, search_for_chunksize=25)
    location_df = convert_key_word_posts_to_df(total_posts, locations, general_re, name_of_search_for = 'locations', sep='|', case_sensitive=False, search_for_chunksize=25)
    
    drug_pricing_df = pd.merge(state_init_df, location_df, on='col', validate='1:1')
    drug_pricing_df = pd.merge(drug_pricing_df, price_df, on='col', validate='1:1')
    drug_pricing_df = pd.merge(drug_pricing_df, unit_df, on='col', validate='1:1')

    meth_df = convert_key_word_posts_to_df(total_posts, meth_words, general_re, name_of_search_for = 'methadone', sep='|', case_sensitive=False, search_for_chunksize=25)
    sub_df = convert_key_word_posts_to_df(total_posts, sub_words, general_re, name_of_search_for = 'suboxone', sep='|', case_sensitive=False, search_for_chunksize=25)
    nalt_df = convert_key_word_posts_to_df(total_posts, nalt_words, general_re, name_of_search_for = 'naltrexone', sep='|', case_sensitive=False, search_for_chunksize=25)
    narc_df = convert_key_word_posts_to_df(total_posts, narc_words, general_re, name_of_search_for = 'naloxone', sep='|', case_sensitive=False, search_for_chunksize=25)


    mat_df = pd.merge(meth_df, sub_df, on='col')
    mat_df = pd.merge(mat_df, nalt_df, on='col')
    mat_df = pd.merge(mat_df, narc_df, on='col')
   ...
   location_unit_dict, filtered_location_unit_dict = merge_dictionaries_by_key(location_dict, unit_dict)
    ...
    total_posts_location_df = convert_key_word_posts_to_df(total_posts, locations)
...
def convert_key_word_posts_to_df(list_of_strings, search_for, regexp, name_of_search_for = '', sep='|', case_sensitive=False, search_for_chunksize=25):
    """
    """
    print('Number of strings searched: %s' % len(list_of_strings))
    print('Number of keywords searching for: %s' % len(search_for))
    ...
    if not case_sensitive:
        ...
        flag = False
    df_name = pd.DataFrame({'col':list_of_strings})
    i, total_chunks = 0, len([x for x in chunks(search_for, search_for_chunksize)])
    for chunk in chunks(search_for, search_for_chunksize):
        i += 1
        print('Chunk %s out of %s' % (i, total_chunks))
        ...
        keywords_grep = sep.join(chunk)
        word = re.compile(regexp.format(keywords_grep), flags=flag)
        df_name[i] = df_name.astype(str).sum(axis=1).str.contains(word, regex=True)
    df_name[name_of_search_for] = df_name.sum(axis=1)
    df_out = df_name[['col',name_of_search_for]]
    return df_out, df_name
...
for reviews,name in zip(total_post,meth_words):
    raw_str = total_posts_df.loc[name].str.cat(sep=',')
